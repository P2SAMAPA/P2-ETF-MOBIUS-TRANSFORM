...
st.set_page_config(page_title="Möbius Transform of Correlation Matrix", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Möbius Transform of Correlation Matrix</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Fractional linear transform of eigenvalues | Macro‑sensitivity of spectral norm</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Möbius")
...
st.sidebar.markdown(f"**Epsilon:** {config.MOBIUS_EPSILON} | **All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'mobius_transform_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Möbius sensitivity: {etf['mobius_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Sensitivity"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Sensitivity", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Möbius Transform Sensitivity (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Möbius transformation** applies a fractional linear transform to correlation eigenvalues.
        - The transform parameter (c) is a function of the composite macro factor.
        - The score is the **sensitivity of the transformed spectral norm** to macro changes.
        - High sensitivity → ETF's correlation structure is highly responsive to macro → potential alpha.
        - Low sensitivity → correlation structure is stable under macro changes.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Möbius sensitivity scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Möbius Transform | Macro‑sensitive spectral analysis")
